...
.backImg {
	background: linear-gradient(rgba(0, 0, 0, .4), rgba(0, 0, 0, .4)),
		url("/HospitalManagementSystem/Images/hospital_5.png");
	height: 20vh;
	width: 100%;
				...
				<div class="card paint-card">
					<div class="card-body">
						...
						<c:if test="${not empty errorMsg}">
							<p class="fs-4 text-center text-danger fw-bold font-monospace">${errorMsg}</p>
							<c:remove var="errorMsg" scope="session" />
						</c:if>
						<c:if test="${not empty succMsg}">
							<p class=" fs-4 text-center text-success fw-bold font-monospace">${succMsg}</p>
							<c:remove var="succMsg" scope="session" />
						</c:if>
						<form class="row g-3" action="/HospitalManagementSystem/AppointmentServlet" method="post">
							...
							<div class="col-md-6">
									<label>Full Name</label> <input type="text" value="${userObj.fullName}"
									readonly class="form-control" name="fullname">
							</div>
							...
							<div class="col-md-6">
									<label>Email</label> <input type="text" value="${userObj.email}"
